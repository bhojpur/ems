...
# This script bootstraps an Bhojpur EMS cluster in AWS EC2 and runs benchmarks.
#
...
#
# AWS EC2 instances are launched into EC2 Classic, expecting a 'default' security
# group that allows allows SSH (port 22) from 0.0.0.0/0 and an EC2 key pair created
...
def _bootstrap(addr):
    ...
    for cmd in [
            ...
            'mkdir -p go/src/github.com/bhojpur',
            'cd go/src/github.com/bhojpur && git clone https://github.com/bhojpur/ems',
            'cd go/src/github.com/bhojpur/ems && git checkout %s' % commit,
            'cd go/src/github.com/bhojpur/ems/apps/emsd && GO111MODULE=on /usr/local/go/bin/go build',
            'cd go/src/github.com/bhojpur/ems/bench/bench_writer && GO111MODULE=on /usr/local/go/bin/go build',
            'cd go/src/github.com/bhojpur/ems/bench/bench_reader && GO111MODULE=on /usr/local/go/bin/go build',
            'sudo -S mkdir -p /mnt/ems',
            'sudo -S chmod 777 /mnt/ems']:
...
def bootstrap():
    ...
    total_count = tornado.options.options.emsd_count + tornado.options.options.worker_count
    ...
    for instance in instances:
        if not instance.tags:
            instance.create_tags(Tags=[{'Key': 'ems_bench', 'Value': '1'}])
...
def run():
    ...
    logging.info('launching emsd on %d host(s)', tornado.options.options.emsd_count)

    emsd_chans = []
    emsd_hosts = instances[:tornado.options.options.emsd_count]
    for instance in emsd_hosts:
        try:
            ssh_client = ssh_connect_with_retries(instance.public_dns_name)
            for cmd in [
                    'sudo -S pkill -f emsd',
                    'sudo -S rm -f /mnt/ems/*.dat',
                    'GOMAXPROCS=32 ./go/src/github.com/bhojpur/ems/apps/emsd/emsd \
                        --data-path=/mnt/ems \
                        --mem-queue-size=10000000 \
                        --max-rdy-count=%s' % (tornado.options.options.rdy)]:
                emsd_chans.append((ssh_client, ssh_cmd_async(ssh_client, cmd)))
        except Exception:
            logging.exception('failed')

    emsd_tcp_addrs = [i.public_dns_name for i in emsd_hosts]

    dt = datetime.datetime.utcnow()
    deadline = dt + datetime.timedelta(seconds=30)

    logging.info('launching %d producer(s) on %d host(s)',
                 tornado.options.options.emsd_count * tornado.options.options.worker_count,
                 tornado.options.options.worker_count)

    worker_chans = []

    producer_hosts = instances[tornado.options.options.emsd_count:]
    for instance in producer_hosts:
        for emsd_tcp_addr in emsd_tcp_addrs:
            topic = hashlib.md5(instance.public_dns_name.encode('utf-8')).hexdigest()
            try:
                ssh_client = ssh_connect_with_retries(instance.public_dns_name)
                for cmd in [
                        'GOMAXPROCS=2 \
                            ./go/src/github.com/bhojpur/ems/bench/bench_writer/bench_writer \
                            --topic=%s --emsd-tcp-address=%s:4150 --deadline=\'%s\' --size=%d' % (
                            topic, emsd_tcp_addr, deadline.strftime('%Y-%m-%d %H:%M:%S'),
    ...
    if tornado.options.options.mode == 'pubsub':
        ...
        consumer_hosts = instances[tornado.options.options.emsd_count:]
        for instance in consumer_hosts:
            for emsd_tcp_addr in emsd_tcp_addrs:
                ...
                try:
                    ...
                    for cmd in [
                            'GOMAXPROCS=8 \
                                ./go/src/github.com/bhojpur/ems/bench/bench_reader/bench_reader \
                                --topic=%s --emsd-tcp-address=%s:4150 --deadline=\'%s\' --size=%d \
                                --rdy=%d' % (
                                topic, emsd_tcp_addr, deadline.strftime('%Y-%m-%d %H:%M:%S'),
    ...
    for ssh_client, chan in emsd_chans:
        chan.close()
...
def _find_instances():
    ...
    return [i for i in ec2.instances.all() if
            i.state['Name'] == 'running' and any(t['Key'] == 'ems_bench' for t in i.tags)]
...
if __name__ == '__main__':
    tornado.options.define('region', type=str, default='us-west-2',
                           help='EC2 region to launch instances')
    tornado.options.define('emsd_count', type=int, default=3,
                           help='how many EMSd instances to launch')
    tornado.options.define('worker_count', type=int, default=3,
                           help='how many worker instances to launch')
    # ubuntu 18.04 HVM instance store us-west-2
